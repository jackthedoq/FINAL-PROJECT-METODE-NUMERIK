% Program Penyelesaian Persamaan Diferensial dengan Metode Runge-Kutta Orde 2 (Heun)
...
% Metode Runge-Kutta Orde 2 (Heun)
for i = 1:N
    k1 = h * f(x_numeric(i), y_numeric(i));
    k2 = h * f(x_numeric(i) + h, y_numeric(i) + k1);
    
    x_numeric(i+1) = x_numeric(i) + h;
    y_numeric(i+1) = y_numeric(i) + 0.5 * (k1 + k2);
end
...
title('Persamaan Diferensial dengan Metode Runge-Kutta Orde 2 (Heun)');
xlabel('x');
ylabel('y(t)');
grid on;
...
xlswrite('data_heun.xlsx', col_header, 'Sheet1', 'A1');
xlswrite('data_heun.xlsx', data, 'Sheet1', 'A2');
