% Program Penyelesaian Persamaan Diferensial dengan Metode Runge-Kutta Orde 3
...
for i = 1:N
    ...
    k3 = h * f(x_numeric(i) + h, y_numeric(i) - k1 + 2*k2);
    
    x_numeric(i+1) = x_numeric(i) + h;
    y_numeric(i+1) = y_numeric(i) + (1/6) * (k1 + 4*k2 + k3);
end
...
title('Persamaan Diferensial dengan Metode Runge-Kutta Orde 3');
xlabel('x');
ylabel('y(t)');
grid on;
...
xlswrite('data_rk3.xlsx', col_header, 'Sheet1', 'A1');
xlswrite('data_rk3.xlsx', data, 'Sheet1', 'A2');
