% Program Penyelesaian Persamaan Diferensial dengan Metode Runge-Kutta Orde 4
...
for i = 1:N
    ...
    k2 = h * f(x_numeric(i) + 0.5 * h, y_numeric(i) + 0.5 * k1);
    k3 = h * f(x_numeric(i) + 0.5 * h, y_numeric(i) + 0.5 * k2);
    k4 = h * f(x_numeric(i) + h, y_numeric(i) + k3);
    
    x_numeric(i+1) = x_numeric(i) + h;
    y_numeric(i+1) = y_numeric(i) + (1/6) * (k1 + 2*k2 + 2*k3 + k4);
end
...
title('Persamaan Diferensial dengan Metode Runge-Kutta Orde 4');
xlabel('x');
ylabel('y(t)');
grid on;
...
xlswrite('data2.xlsx', col_header, 'Sheet1', 'A1');
xlswrite('data2.xlsx', data, 'Sheet1', 'A2');
